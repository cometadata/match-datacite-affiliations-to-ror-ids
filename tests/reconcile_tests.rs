...
fn write_creator_occurrences(dir: &std::path::Path, relationships: &[AuthorAffiliationRecord]) {
    ...
    for relationship in relationships {
        let occurrence = AffiliationOccurrenceRecord {
            ...
            party_index: relationship.author_idx,
            party_name: Some(relationship.author_name.clone()),
            party_name_type: relationship.author_name_type.clone(),
            party_given_name: relationship.author_given_name.clone(),
            party_family_name: relationship.author_family_name.clone(),
            party_raw: None,
            affiliation_index: relationship.affiliation_idx,
...
fn test_load_ror_matches_builds_hash_map() {
    ...
    assert_eq!(
        lookup.get("def456"),
        Some(&"https://ror.org/042nb2s44".to_string())
    );
}
...
#[test]
fn test_reconcile_requires_canonical_occurrence_stream() {
    let temp_dir = TempDir::new().unwrap();
    let input_dir = temp_dir.path().join("input");
    std::fs::create_dir_all(&input_dir).unwrap();
    let mut legacy_file = File::create(input_dir.join("doi_author_affiliations.jsonl")).unwrap();
    writeln!(legacy_file, "{{}} ").unwrap();
    File::create(input_dir.join("ror_matches.jsonl")).unwrap();

    let args = datacite_ror::reconcile::ReconcileArgs {
        input: input_dir,
        output: Some(temp_dir.path().join("output.jsonl")),
        ror_data: create_minimal_ror_data(temp_dir.path()),
        enrichment_format: false,
        enrichment_config: None,
    };

    assert!(datacite_ror::reconcile::run(args).is_err());
}
...
fn test_reconcile_full_pipeline() {
    ...
    // Create canonical creator occurrence records.
    ...
    write_creator_occurrences(&input_dir, &relationships);
    let contributor = AffiliationOccurrenceRecord {
        doi: "10.1234/test".to_string(),
        party_type: PartyType::Contributor,
        party_index: 0,
        party_name: Some("Contributor, Casey".to_string()),
        party_name_type: None,
        party_given_name: None,
        party_family_name: None,
        party_raw: None,
        affiliation_index: 0,
        affiliation: "University of Oxford".to_string(),
        affiliation_hash: "abc123".to_string(),
    ...
    let canonical_file = std::fs::OpenOptions::new()
        .append(true)
        .open(input_dir.join("doi_affiliation_occurrences.jsonl"))
        .unwrap();
    writeln!(
        std::io::BufWriter::new(canonical_file),
        "{}",
        serde_json::to_string(&contributor).unwrap()
    )
    .unwrap();
    ...
    assert_eq!(
        record.creators[0].affiliation[0].name,
        "University of Oxford"
    );
    assert_eq!(
        record.creators[0].affiliation[0].affiliation_identifier,
        "https://ror.org/052gg0110"
    );
    assert_eq!(
        record.creators[0].affiliation[0].affiliation_identifier_scheme,
        "ROR"
    );
    assert_eq!(
        record.creators[0].affiliation[0].scheme_uri,
        "https://ror.org"
    );
    ...
    assert_eq!(
        record.creators[1].affiliation[0].affiliation_identifier,
...
fn test_reconcile_skips_doi_with_no_matches() {
    ...
    let relationships = vec![AuthorAffiliationRecord {
...
fn test_load_ror_data_builds_name_lookup() {
    ...
    assert_eq!(lookup.len(), 2);
    assert_eq!(
        lookup.get("https://ror.org/052gg0110"),
        Some(&"University of Oxford".to_string())
    );
    assert_eq!(
        lookup.get("https://ror.org/042nb2s44"),
        Some(&"Massachusetts Institute of Technology".to_string())
    );
}
...
fn test_load_ror_data_handles_missing_ror_display() {
    ...
    assert_eq!(
        lookup.get("https://ror.org/test123"),
...
fn test_reconcile_enrichment_format() {
    ...
    // Create canonical creator occurrence records for two authors on one DOI.
    let relationships = vec![
        AuthorAffiliationRecord {
            doi: "10.1234/test".to_string(),
            author_idx: 0,
...
"#,
    ...
    assert_eq!(
        affiliations[0]["affiliationIdentifier"],
        "https://ror.org/052gg0110"
    );
    ...
    assert_eq!(
        resources[0]["relatedIdentifier"],
        "http://doi.org/10.82461/160e-8q92"
    );
    ...
    assert_eq!(
        rec2["enrichedValue"]["affiliation"][0]["affiliationIdentifier"],
        "https://ror.org/042nb2s44"
    );
}

#[test]
fn test_canonical_creator_enrichment_preserves_raw_payloads() {
    let temp_dir = TempDir::new().unwrap();
    let input_dir = temp_dir.path().join("input");
    let output_file = temp_dir.path().join("enrichments.jsonl");
    std::fs::create_dir_all(&input_dir).unwrap();

    let party_raw = serde_json::json!({
    ...
    let occurrences = vec![
        AffiliationOccurrenceRecord {
            doi: "10.1234/raw".to_string(),
            party_type: PartyType::Creator,
            party_index: 0,
            party_name: Some("Doe, Jane".to_string()),
            party_name_type: None,
            party_given_name: None,
            party_family_name: None,
            party_raw: Some(party_raw.clone()),
            affiliation_index: 0,
            affiliation: "University of Oxford".to_string(),
        ...
        AffiliationOccurrenceRecord {
            ...
            party_given_name: None,
            party_family_name: None,
            party_raw: Some(party_raw),
            affiliation_index: 1,
            affiliation: "Some Lab".to_string(),
            affiliation_hash: "lab".to_string(),
    ...
    let mut occurrence_file =
        File::create(input_dir.join("doi_affiliation_occurrences.jsonl")).unwrap();
    ...
    writeln!(File::create(input_dir.join("ror_matches.jsonl")).unwrap(), r#"{{"affiliation":"University of Oxford","affiliation_hash":"oxford","ror_id":"https://ror.org/052gg0110"}}"#).unwrap();
    let config = temp_dir.path().join("config.yaml");
    std::fs::write(&config, "contributors: []\nresources: []\n").unwrap();

    datacite_ror::reconcile::run(datacite_ror::reconcile::ReconcileArgs {
        input: input_dir,
        output: Some(output_file.clone()),
        ror_data: create_minimal_ror_data(temp_dir.path()),
        enrichment_format: true,
    ...
    assert_eq!(output["originalValue"]["lang"], "en");
    assert!(output["enrichedValue"]["nameIdentifiers"].is_array());
    assert_eq!(
        output["enrichedValue"]["affiliation"][1]["affiliationIdentifierScheme"],
        "ISNI"
    );
}
